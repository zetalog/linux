...
#define CLK_SBI_DEV_SIZE(nr_clks)	\
	(sizeof(struct __sbi_device) +	\
	 ((nr_clks) * sizeof(struct clk_hw *)))

#define clk_hw_to_sbi_clk(pch)		\
	container_of(pch, struct sbi_clk, hw)
...
{
	struct sbi_clk *clk = clk_hw_to_sbi_clk(hw);

	return sbi_get_clk_freq(clk->clkid);
}
				 ...
				 struct __sbi_device *device)
{
	...
	for (i = 0; i < drv->nr_clks; ++i) {
		...
		ret = clk_hw_register_clkdev(&clk->hw, clk->name,
					     dev_name(dev));
		if (ret) {
...
static int sbi_clock_probe(struct platform_device *pdev)
{
	...
	device = devm_kzalloc(dev, CLK_SBI_DEV_SIZE(driver->drv->nr_clks),
			      GFP_KERNEL);
	if (!device) {
	...
	dev_info(dev, "SBI clock %s(%lu) probed %d clocks\n",
		 driver->drv->name, driver->drv->type,
		 device->hw_clks.num);

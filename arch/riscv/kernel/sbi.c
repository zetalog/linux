...
static unsigned long __sbi_get_clk_freq(unsigned long clkid)
{
	...
	ret = sbi_ecall(SBI_EXT_CLK, SBI_EXT_CLK_GET_FREQUENCY,
			clkid, 0, 0, 0, 0, 0);
	return ret.error;
}
...
static void __sbi_set_clk_freq(unsigned long clkid, unsigned long freq)
{
	sbi_ecall(SBI_EXT_CLK, SBI_EXT_CLK_SET_FREQUENCY,
		  clkid, freq, 0, 0, 0, 0);
}

static void __sbi_enable_clk(unsigned long clkid)
{
	sbi_ecall(SBI_EXT_CLK, SBI_EXT_CLK_ENABLE, clkid, 0, 0, 0, 0, 0);
}
